...
- `start_minecraft_server` без запису в `MINECRAFT_SERVERS` -> `status=not_configured`, `reason_code=minecraft_server_not_configured`.
...
У v0.4 Minecraft server навмисно не налаштований. Якщо `MINECRAFT_SERVERS` порожній, router повертає `status=not_configured`, `reason_code=minecraft_server_not_configured` і нічого не запускає. Для майбутнього запуску потрібно явно додати запис у `MINECRAFT_SERVERS` в `actions/apps.py`, наприклад:
